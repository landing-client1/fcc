...
def get_para_time(root_path, bmark, times, num_workers=32):
...
def get_real_speedup(root_path, bmark, reg_option, times=3, default_num_worker=8):
    os.chdir(os.path.join(root_path, bmark, "src"))

    num_workers_plan_list = list(range(1, 33))  # [1,32]

    real_speedup = {}
    # if reg_option == 0:
    #     print("Compile executable automatic parallelization program for %s " % (bmark))
    #     start_time = time.time()
    #     make_process = subprocess.Popen(["make", "reg_para", "REG_NUM_WORKERS=" + str(20)],
    #                                     stdout=subprocess.DEVNULL,
    #                                     stderr=subprocess.STDOUT)
    #     elapsed = time.time() - start_time
    #     if make_process.wait() != 0 and os.path.exists(para_time_name):
    ...
    #         return False
    #     else:
    #         print(colored("Compile successful, took %.4fs" % (elapsed), 'green'))
    # else:
    ...
    # Parallel time only
        ...
        for num_workers in num_workers_plan_list:
            ...
            print("%s %.3f on sequential" % (bmark, seq_time))
            print("%s %.3f on %d workers" % (bmark, para_time, num_workers))
            if seq_time and para_time and seq_time > 0 and para_time > 0:
                speedup = seq_time / para_time
                print(colored("%s speedup over sequential on %d workers is: %.3f" % (bmark, num_workers,speedup), 'yellow'))
...
def parse_args():
    ...
    # bmark_list = []
    # bmark_list.append(args.bmark_list)
